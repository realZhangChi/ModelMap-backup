...
namespace ModelMap.IdentityServer
{
    public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        ...
        {
            await CreateApiScopeAsync("ModelMap");
        }
        ...
        private async Task CreateApiResourcesAsync()
        {
            ...
            await CreateApiResourceAsync("ModelMap", commonApiUserClaims);
        }
        ...
        private async Task CreateClientsAsync()
        {
            ...
            var consoleAndAngularClientId = configurationSection["ModelMap_App:ClientId"];
            if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
            {
                var webClientRootUrl = configurationSection["ModelMap_App:RootUrl"]?.TrimEnd('/');

                await CreateClientAsync(
                    name: consoleAndAngularClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                    secret: (configurationSection["ModelMap_App:ClientSecret"] ?? "1q2w3e*").Sha256(),
            ...
            {
                var swaggerRootUrl = configurationSection["ModelMap_Swagger:RootUrl"].TrimEnd('/');

                await CreateClientAsync(
                    name: swaggerClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "authorization_code" },
                    secret: configurationSection["ModelMap_Swagger:ClientSecret"]?.Sha256(),
                    requireClientSecret: false,
